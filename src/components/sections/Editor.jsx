    ...
    return (
        <div className='flex h-full w-full bg-bg-secondary dark:bg-dark-bg-secondary overflow-hidden border border-border dark:border-dark-border rounded-lg shadow-2xl transition-all'>
            ...
            <main className='flex-1 flex flex-col min-w-0 bg-bg-primary dark:bg-dark-bg-primary text-text-primary dark:text-dark-text-primary transition-colors'>
                <div className='flex bg-bg-secondary/50 dark:bg-dark-bg-secondary/50 border-b border-border dark:border-dark-border transition-colors overflow-x-auto custom-scrollbar whitespace-nowrap'>
                    {projectsData.map(project => (
                        <div
                            key={project.id}
                            onClick={() => setSelectedId(project.id)}
                            className={`px-4 py-2 text-xs font-mono border-r border-border dark:border-dark-border cursor-pointer flex items-center gap-2 shrink-0 transition-colors ${selectedId === project.id ? 'bg-bg-primary dark:bg-dark-bg-primary text-text-primary dark:text-dark-text-primary border-t-2 border-t-arch-blue' : 'text-muted hover:bg-black/5 dark:hover:bg-white/5'
                                }`}
                        >
                            <FileCode size={12} />
                            {project.id}
                            <X size={10} className='hover:text-red-500 opacity-50' />
                        </div>
                    ))}
                </div>

                <div className='flex-1 p-8 overflow-y-auto custom-scrollbar relative bg-transparent'>
                        ...
                        {selectedProject && (
                            ...
                            >
                                <div className="flex items-center gap-4 mb-6">
                                    <div className="p-3 bg-arch-blue/10 rounded-lg text-arch-blue">
                                        <FileCode size={32} />
                                    </div>
                                    <div>
                                        <h2 className='text-3xl font-bold'>{selectedProject.title}</h2>
                                        <p className='text-muted font-mono text-xs uppercase tracking-widest mt-1'>
                                            {selectedProject.tags[0]} | ~/projects/{selectedProject.id}
                                        </p>
                                    </div>
                                </div>

                                <div className='space-y-6'>
                                    <div className='p-4 bg-black/5 dark:bg-white/5 rounded-md font-mono text-sm leading-relaxed border-l-4 border-arch-blue transition-colors'>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {selectedProject.links.github && (
                                            ...
                                            >
                                                <Github size={16} />View {selectedProject.title} on GitHub
                                            </a>
                                        )}
                                    ...
                                    <div className='pt-4'>
                                        <h4 className='text-xs font-mono text-arch-blue uppercase mb-3'>Tech Stack</h4>
                                        <div className='flex flex-wrap gap-2'>
                                            {selectedProject.tags.map(tag => (
                                                <span key={tag} className='px-3 py-1 bg-black/5 dark:bg-white/5 rounded-full text-[10px] font-mono text-muted border border-border dark:border-dark-border'>
                                                    {tag}
                                                </span>
                                            ))}
                                        </div>
                ...
                <div className='flex items-center text-[10px] font-bold transition-colors'>
                    <div className='bg-arch-blue text-black px-4 py-1'>NORMAL</div>
                    <div className='bg-bg-secondary dark:bg-[#1c1c1c] text-muted px-4 py-1 flex-1 transition-colors'>
                        ~/meetsept/projects/{selectedId}
                    </div>
                    <div className='bg-bg-secondary dark:bg-[#1c1c1c] text-arch-blue px-4 py-1 border-l border-border dark:border-white/10 transition-colors'>UTF-8</div>
                    <div className='bg-arch-blue text-black px-4 py-1 uppercase'>{selectedProject?.tags[0] || 'FILE'}</div>
                </div>
            </main>
        </div>
    );
